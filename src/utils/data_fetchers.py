...
def get_test_features(
    ...
    with open(avg_train_features_path, "rb") as stream:
        ...
        # if feature values are numpy arrays, convert them to torch tensors
        if isinstance(train_features, dict):
            for key, value in train_features.items():
                if isinstance(value, np.ndarray):
                    train_features[key] = torch.from_numpy(value)
        ...
        if len(train_features) == 2:
            ...
            # only used for stats so order does not matter
            concat_features = torch.concat(list(train_features.values()))  
            average_train_features = concat_features.mean(0).unsqueeze(0)
            std_train_features = concat_features.std(0).unsqueeze(0)
